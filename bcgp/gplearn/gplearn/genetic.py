...
def get_candidates(tournament_size, num_tournaments=10, random_state=None):
    """Get candidates by tournament selection"""
    ...
    for _ in range(num_tournaments):
        contenders = random_state.randint(0, len(populations), tournament_size)
        fitness = [populations[p].fitness_ for p in contenders]
        parent_index = contenders[np.argmin(fitness)]
        parent = populations[parent_index]
        
        candidates.append(parent)
        candidates_y.append(parent.y_predicts)
    
    return candidates, candidates_y

def _parallel_evolve(n_programs, op_quota, parents, X, y, sample_weight, seeds, params):
    ...
    for i in range(n_programs):
        ...
        if parents is None:
            ...
            if method < method_probs[0]:
                ...
                donor_y_predicts = donor.y_predicts
                receiver_fitness = parent.raw_fitness(X, y, np.ones((n_samples,)))
                program, removed, remains, crossover_op_count, operator_data, subtree_data, receiver_data, donor_data = parent.crossover(dim, predict, donor.program, donor_y_predicts, random_state, op_quota)
                check_program = _Program(function_set=function_set,
                                arities=arities,
                                ...
                                program=program)
                crossover_fitness = check_program.raw_fitness(X, y, np.ones((n_samples,)))
                append_data = []
                append_data = operator_data + subtree_data + donor_data
                global mlp_data
                mlp_data.append(append_data)
                genome = {'method': 'Crossover',
                          ...
                          'donor_nodes': remains}
                global record
                if crossover_fitness < 1e-5:
                    record = True
                    all_op_counts += crossover_op_count
                if record != True:
                    all_op_counts += crossover_op_count
                if all_op_counts >= op_quota:
                    break
                ...
                # subtree_mutation
                program, removed, remains, submut_op_count, operator_data, subtree_data, receiver_data, donor_data = parent.subtree_mutation(function_set, arities, init_depth, init_method, n_features, metric, transformer, const_range, p_point_replace, parsimony_coefficient, feature_names, dim, predict, X, random_state, op_quota)
                ...
                check_program = _Program(function_set=function_set,
                                ...
                                random_state=random_state,
                                program=program)
                submut_fitness = check_program.raw_fitness(X, y, np.ones((n_samples,)))
                append_data = []
                append_data = operator_data + subtree_data + donor_data
                mlp_data.append(append_data)
                # global record
                if submut_fitness < 1e-5:
                    record = True
                    all_op_counts += submut_op_count
                if record != True:
                    all_op_counts += submut_op_count
                if all_op_counts >= op_quota:
                    break
                ...
                # hoist_mutation
                program, removed, hoistmut_op_count = parent.hoist_mutation(random_state, op_quota)
                ...
                check_program = _Program(function_set=function_set,
                                ...
                                random_state=random_state,
                                program=program)
                hoistmut_fitness = check_program.raw_fitness(X, y, np.ones((n_samples,)))
                # global record
                if hoistmut_fitness < 1e-5:
                    record = True
                    all_op_counts += hoistmut_op_count
                if record != True:
                    all_op_counts += hoistmut_op_count
                if all_op_counts >= op_quota:
                    break
                ...
                # point_mutation
                program, mutated, ptmut_op_count = parent.point_mutation(random_state, op_quota)
                ...
                check_program = _Program(function_set=function_set,
                                ...
                                random_state=random_state,
                                program=program)
                ptmut_fitness = check_program.raw_fitness(X, y, np.ones((n_samples,)))
                # global record
                if ptmut_fitness < 1e-5:
                    record = True
                    all_op_counts += ptmut_op_count
                if record != True:
                    all_op_counts += ptmut_op_count
                if all_op_counts >= op_quota:
                    break
        ...
        # Deal with the None Type genome when the op quota is reached
        ...
        if parents is None: # gen = 0
            from ._program import op_count
            all_op_counts += op_count
...
class BaseSymbolic(BaseEstimator, metaclass=ABCMeta):
    ...
    def fit(self, X, y, sample_weight=None):
        ...
        for gen in range(prior_generations, self.generations):
            ...
            print(f"OP: {int(all_op_counts)}")
            if all_op_counts >= int(self.op) or stop:
                break
            ...
            # print(f'all_op_counts: {all_op_counts}')
            self.final_op_count = all_op_counts
            self.costgen = gen
            ...
            if gen > 0:
                ...
                mlp = MLPTrainer(device="cuda", gpu_id=0)
                mlp_x = [row[:3] for row in mlp_data]
                mlp_y = [row[3:] for row in mlp_data] 
                mlp.train(mlp_x, mlp_y)
                global predict
                predict = True
            ...
            # Check for early stopping (added)
            avg_fitness = 0
            min_fitness = 1e6
            threshold = 1e-8
            ...
            avg_fitness /= len(fitness)
            if abs(min_fitness - avg_fitness) < threshold:
                break 
